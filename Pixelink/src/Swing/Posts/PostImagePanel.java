...
public class PostImagePanel extends JPanel {
	...
	public PostImagePanel(Usuario usuario, PostImage post) {
	...
	private void setupComponents() {
		...
		if (post.getConteudo().getImage() != null) {
			ImageIcon imageIcon = new ImageIcon(
					usuario.getFotoDoPerfil().getImage().getScaledInstance(50, 50, Image.SCALE_SMOOTH));
						...
						if (SwingUtilities.getWindowAncestor(imageLabel) instanceof Frame) {
							...
							new DialogImagem(
									(Frame) SwingUtilities
											.getWindowAncestor(SwingUtilities.getWindowAncestor(imageLabel)),
									usuario.getFotoDoPerfil(), 2.5);
						}
		...
		IdLabel.addMouseListener(new MouseAdapter() {
			...
			@Override
			public void mouseExited(MouseEvent e) {
				IdLabel.setCursor(Cursor.getDefaultCursor());
				IdLabel.setForeground(Color.WHITE);
				Font font = IdLabel.getFont();
				Map<TextAttribute, Object> attributes = new HashMap<>(font.getAttributes());
		...
		imageContentLabel.setBackground(Color.BLACK);

		if (post.getTexto() != null) {
			if (post.getConteudo().getImage() != null) {
				JLabel imageLabel = new JLabel(
						new ImageIcon(post.getConteudo().getImage().getScaledInstance(400, 400, Image.SCALE_SMOOTH)));
				imageLabel.addMouseListener(new MouseAdapter() {
					@Override
					public void mouseClicked(MouseEvent e) {
						if (e.getButton() == MouseEvent.BUTTON1) {
				...
				imageContentLabel.add(imageLabel, BorderLayout.NORTH);

				JTextArea textContentArea = new JTextArea(post.getTexto());
				textContentArea.setEditable(false);
				textContentArea.setLineWrap(true);
				textContentArea.setWrapStyleWord(false);
				textContentArea.setFont(Dados.loadFontFromFile(18));
				textContentArea.setMargin(new Insets(5, 5, 5, 5));
				imageContentLabel.add(textContentArea);
				add(imageContentLabel, BorderLayout.CENTER);
			} else {
				JTextArea textContentArea = new JTextArea("infelizmente essa imagem não pode ser carregada");
				textContentArea.setEditable(false);
				textContentArea.setLineWrap(true);
				textContentArea.setWrapStyleWord(true);
				textContentArea.setFont(Dados.loadFontFromFile(18));
				add(textContentArea, BorderLayout.CENTER);
			}
		}
		...
		if (tamanho > 0) {
			for (int i = 0; i < tamanho; i++) {
				...
				ComentarioPost comentario = new ComentarioPost(
						Dados.getUsuario(comentarios.get(i).getUsuario()).getID(),
						comentarios.get(i).getConteudo() + (i + 1),
