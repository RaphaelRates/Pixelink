...
public class CarrocelDePostsOutroUser extends JPanel {
	...
	public CarrocelDePostsOutroUser(Usuario usuario) {
		this.setUsuario(usuario);
		setSize(700, 400);

		painelTitulo = new JPanel();
		painelTitulo.setBackground(Color.decode("#FF0070"));
		...
		if (usuario.getName().split(" ").length > 1) {
			lblTitulo = new JLabel("Publicações de " + usuario.getName().substring(0, usuario.getName().indexOf(" ")));
			
		}else {
			lblTitulo = new JLabel(usuario.getName());
		}

		sl_painelTitulo.putConstraint(SpringLayout.EAST, lblTitulo, -874, SpringLayout.EAST, painelTitulo);
		lblTitulo.setFont(Dados.loadFontFromFile(27));
		lblTitulo.setForeground(Color.white);
		painelTitulo.add(lblTitulo);

		lblCartinha = new JLabel("");
		sl_painelTitulo.putConstraint(SpringLayout.NORTH, lblTitulo, 0, SpringLayout.NORTH, lblCartinha);
		sl_painelTitulo.putConstraint(SpringLayout.WEST, lblTitulo, 18, SpringLayout.EAST, lblCartinha);
		sl_painelTitulo.putConstraint(SpringLayout.SOUTH, lblTitulo, 0, SpringLayout.SOUTH, lblCartinha);
	...
	public JLabel getLblGato() {
		return lblGato;
	}

	public void setLblGato(JLabel lblGato) {
		this.lblGato = lblGato;
	}
}
